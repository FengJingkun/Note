...
为了在发送端调节发送数据的量，定义了**拥塞窗口。**

**拥塞窗口cwnd是发送方维护的一个状态变量，根据网络的拥塞程度动态变化。**

**引入cwnd后，发送窗口的大小变为min( cwnd, 接收窗口 )。**

- 只要网络中未出现拥堵，cwnd就会增大；
- 网络中出现拥塞，cwnd则会减小

只要发送方未在规定时间内收到ACK报文，即发生了超时重传，就认为网络出现了拥塞。

<br>

### 慢启动

TCP建立连接后进入慢启动状态，**每收到一个ACK，cwnd的大小就会加1。**因此慢启动的cwnd大小呈指数增加。

**慢启动门限slow start threshold，ssthresh状态变量**
...
拥塞窗口大于慢启动门限后使用拥塞避免算法。**每收到一个ACK，cwnd的大小增加 1 / cwnd**。

假设ssthresh为8，当8个ACK到来时，每个ACK增加八分之一，8个ACK使得cwnd一共增加1，**此时cwnd的大小变成了线性增长。**因此，拥塞避免实际上是**将cwnd的指数增长变成了线性增长，cwnd仍处于增长阶段，只不过降低了增长速度。**

随着各个TCP连接cwnd的增长，网络开始进入拥塞状态，于是就会出现丢包现象。

触发**超时重传机制**后，就会使用**拥塞发生算法**。

<br>

### 拥塞发生

重传机制主要分为超时重传和快速重传，这两种机制对应的拥塞处理算法有所不同。

- **超时重传的拥塞发生算法**

  ssthresh变为cwnd的二分之一，cwnd重置为1。然后重新进入**慢启动算法**。

- **快速重传的拥塞发生算法**

  cwnd变为原来的二分之一，ssthresh重置为cwnd。然后进入**快恢复算法**。
...
进入快恢复算法前，cwnd = cwnd / 2，ssthresh = cwnd。

进入快恢复算法后：

- cwnd = ssthresh + 3 (连续收到了3个确认号相同的ACK报文)
- 重传丢失的数据包
- 若再次收到重复的ACK包，则cwnd = cwnd + 1
- 若收到新ACK包，则cwnd = ssthresh，直接进入**拥塞避免算法**。